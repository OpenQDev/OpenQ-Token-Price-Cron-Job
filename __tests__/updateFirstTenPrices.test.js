...
const updateTopTenPrices = require('../src/updateTopTenPrices');
		const expectedData = {
...
describe('updateTopTenPrices', () => {


	beforeEach(() => {
		jest.mock('axios');
		axios.get = jest.fn().mockResolvedValue({ data: expectedData });
		axios.post = jest.fn().mockResolvedValue({ data: expectedData });
	});

	it('updateTopTenPrices', async () => {
